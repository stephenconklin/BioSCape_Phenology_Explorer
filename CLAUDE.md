...
| First pixel click after page load: chart tab (e.g. Raw VI) stays blank; switching tabs away and back makes it appear | `Plotly.react()` (what `dcc.Graph` uses to apply a new `figure`) doesn't force a container remeasure on its own — only an explicit `window` `resize` event does, which `config={"responsive": True}` listens for. `resize.js` only dispatches that event once, ~200 ms after page load — enough to size the empty placeholder figure, but nothing re-fires it when the first *real* figure lands after a click. Switching tabs happens to force a remeasure incidentally (`dbc.Tabs` remounts the tab-pane), masking the gap | Added a clientside callback (`app.py`) that dispatches the same `window` `resize` event ~50 ms after a figure actually updates, via a hidden `html.Div(id="_resize-ping")` placeholder Output. **Must be keyed on the chart `figure` Outputs themselves** (`Input("timeseries-chart", "figure")` etc.), not on `Input("pixel-result", "data")` — the four chart callbacks are separate server round-trips from `compute_pixel_result`, so keying on the Store fired the resize in parallel with (not after) those round-trips; over localhost's near-instant round-trip this coincidentally worked, but over real network latency (Jetstream2 deployment) the resize fired before the actual figure had arrived, remeasuring a container that still held the old/empty figure |
